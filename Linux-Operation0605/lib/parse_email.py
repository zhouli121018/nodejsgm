...
class ParseEmail():
    ...
    def parseMailTemplate(self):
        ...
        for part in self.obj.walk():
            ...
            attachment, real_attachment = self._get_attachment(part)
            if attachment:
                attachments.append(attachment)
            if real_attachment:
                real_attachments.append(real_attachment)
        ...
        maila['real_attachments'] = real_attachments
        return maila
    ...
    def _get_attachment(self, part):
        if part.get('Content-Disposition', '').startswith('attachment') or part.get_param('name'):
            ...
            real_attachment = copy.deepcopy(attachment)
            if 'content_id' in attachment:
                attachment['content_id'] = attachment.get('content_id')[1:-1]
                real_attachment = None
            return attachment, real_attachment
        return None, None
    ...
    def replace_sender(self, sender):
        message_object = self.obj
...
if __name__ == "__main__":
    ...
    email_str = open('/home/comingchina/documents/111.eml').read()
    p = ParseEmail(email_str)
    data = p.parseMailTemplate()
    print data.get('from')
    s = p.replace_sender('aa@aa.aa')
