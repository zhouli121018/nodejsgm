...
def get_mail_message(customer_id, mail_to, sender_from, type='customer', interval=0):
    mail_model = get_mail_model(get_mail_date())
    if type == 'customer':
        if interval:
            virus_mails = get_mails_from_interval(customer_id, interval, mail_to=mail_to, check_result='virus')
            other_mails = get_mails_from_interval(customer_id, interval, mail_to=mail_to, check_result='not_virus')
        else:
        ...
        if interval:
            virus_mails = get_mails_from_interval(customer_id, interval, mail_to='', check_result='virus')
            other_mails = get_mails_from_interval(customer_id, interval, mail_to='', check_result='not_virus')
        else:
            ...
            other_mails = mail_model.objects.filter(customer__id=customer_id, state='reject',
                                                    review_result='reject').exclude(check_result='virus').order_by(
                'mail_to')
...
def get_mail(customer_id, mail_to, type='customer', interval=0):
    if type == 'manager':
        try:
    ...
    mail_message = get_mail_message(customer_id, mail_to, sender_from, type=type, interval=interval)
...
def work_send(customer_id, mail_to, type='customer', interval=0):
    """
    ...
    """
    time1 = time.time()
    mail_info = get_mail(customer_id, mail_to, type=type, interval=interval)
    if not mail_info:
        return
    ...
    msg = decode_msg(msg)
    time2 = time.time()
    log.info(u'spam_rpt message info, type={}, mail_to={}, code={}, msg={}, address={}, interval={}, time={}'
             .format(type, mail_info['mail_to'], code, msg, mail_info['addr_host'], interval, time2 - time1))


def _get_mails(mails, check_result, mail_to):
    if check_result == 'virus':
        mails = mails.filter(check_result='virus')
    elif check_result == 'not_virus':
        mails = mails.exclude(check_result='virus')
    if mail_to:
        mails = mails.filter(mail_to=mail_to)
    else:
        mails = mails.order_by('mail_to')
    return list(mails)


def get_mails_from_interval(customer_id, interval, mail_to=None, check_result=None):
    """
    ...
    if start_time.day == end_time.day:
        mails = mail_model_start.objects.filter(customer_id=customer_id, state='reject', mail_to__isnull=False,
                                                review_result='reject', created__gte=start_time, created__lt=end_time)
        mails = _get_mails(mails, check_result, mail_to)
    else:
        first_mails = mail_model_start.objects.filter(customer_id=customer_id, state='reject', mail_to__isnull=False,
                                                      review_result='reject', created__gte=start_time)
        last_mails = mail_model_end.objects.filter(customer_id=customer_id, state='reject', mail_to__isnull=False,
...
def check_interval(customer_id, interval):
    """
    检测是否满足发送间隔
    key: 2018010110
         2018010108
    ...
    key = 'interval_spamrpt:{}'.format(customer_id)
    while redis.llen(key) > 30:
        redis.rpop(key)
    last_time = redis.lindex(key, 0)
    if not last_time or (last_time and (datetime.now() - datetime.strptime('{}0000'.format(last_time),
                                                                                             "%Y%m%d%H%M%S")).seconds >= interval * 3600):
        redis.lpush(key, datetime.now().strftime('%Y%m%d%H'))
        return False
    return True
...
def interval_sendtime():
    ...
    try:
        ...
        for s in CustomerSetting.objects.filter(interval_spamrpt__gt=0).exclude(customer__gateway_status='disabled'):
            interval = int(s.interval_spamrpt)
            customer_id = s.customer_id
            if check_interval(customer_id, interval):
                log.info('check interval fail(customer_id:{}, interval:{})'.format(customer_id, interval))
                continue
            if s.spamrpt:
                mails = get_mails_from_interval(customer_id, interval)
                for mail_to in set([m.mail_to for m in mails]):
                    pool.spawn(work_send, customer_id, mail_to, 'customer', interval)
            if s.m_spamrpt:
...
if __name__ == '__main__':
    ...
    lib.common.init_logger('IntervalSpamRpt', _DEBUG, _DEBUG)
