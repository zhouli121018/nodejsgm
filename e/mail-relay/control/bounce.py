...
from deliver.lib.models import log_model, bounce_model
from deliver.lib.utility import decode_msg
from apps.core.models import CustomerSetting
from apps.mail.models import get_mail_model, BounceSettings
...
def bounce(j):
    ...
    while True:
        try:
            ...
            try:
                enable = CustomerSetting.objects.filter(customer=mail.customer, bounce=True).first()
    ...
    (redis.pipeline()
     .lpush('control_bounced', json.dumps(bounce_model.dump(d)))
     .lrem('control_bounce_waiting', 0, j)
     .execute())
...
def put_routine():
    ...
    while True:
        j = redis.brpoplpush('control_bounce', 'control_bounce_waiting')
        pool.spawn(bounce, j)
...
def sync_db():
    while True:
        j = redis.brpoplpush('control_bounced', 'control_bounced_temp')
        # d = bounce_model.load(json.loads(j))
        d = json.loads(j)
        save(d)
        redis.lrem('control_bounced_temp', 0, j)
        log.info(u'sync_db: saved to postgres, mail_ident={}'.format(d['mail_ident']))


def init():
    while redis.rpoplpush('control_bounce_waiting', 'control_bounce') is not None:
        pass
    while redis.rpoplpush('control_bounced_temp', 'control_bounced') is not None:
        pass
