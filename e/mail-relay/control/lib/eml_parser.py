...
url_regex = re.compile(
    r'''(?i)\b((?:(hxxps?|https?|ftps?)://|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'".,<>?]))''',
...
def get_raw_body_text(msg):
    ...
    for part in msg.walk():
        ...
        print subtype
        body_orig = part.get_payload()
        if subtype == 'plain':
            plain_text = body_orig
        if subtype == 'html' and body_orig: break

    if not body_orig:
        body_orig = plain_text

    # 解码内容
    # body_code = part.get('Content-Transfer-Encoding')
    body_code = part.get('content-transfer-encoding', '').lower()
    if body_code == 'base64':
        body_text = base64.decodestring(body_orig)
    elif body_code == 'quoted-printable':
        body_text = quopri.decodestring(body_orig)
    else:
        body_text = body_orig
    # body_text = _convertCharset(body_text, charset)

    return body_text

# 转换指定字符串的字符编码
def _convertCharset(string, from_charset, to_charset='utf-8'):
...
def q_value_decode(string):
    ...
    if m:
        ...
        d_string = string.decode('utf-8', 'ignore')
...
def main():
    ...
    m = decode_email(msgfile, include_attachment_data=True)
