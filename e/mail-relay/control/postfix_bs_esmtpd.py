...
class SMTPChannel(smtpd.SMTPChannel):
    def __init__(self, server, conn, addr, user_validator=None, ip_validator=None, domain_validator=None,
                 collect_domain_validator=None, mailbox_validator=None):
        ...
        self.mailbox_validator = mailbox_validator
    ...
    def smtp_XFORWARD(self, arg):
        ...
        if key == 'ADDR':
            self.senderip = val
            if not self.authenticated:
                disabled, uid = self.ip_validator(self.senderip)
                if disabled == '0':
                    self.uid = uid
                    self.user_validator = None
                    self.authenticated = True
    ...
    def found_terminator(self):
        ...
        if self.__state == self.COMMAND:
            ...
            if not self.authenticated:
                p = ParseEmail(self.__data)
                username = p.get_login_username()
                disabled, uid = self.mailbox_validator(username)
                self.uid = uid
                self.authenticated = True
            ...
            # if not self.uid:
            if not self.uid:
                disabled, uid = self.domain_validator(self.__mailfrom.split('@')[-1])
                if disabled == '0':
                    self.uid = uid
                else:
                    disabled, uid = self.ip_validator(self.senderip)
...
class SMTPServer(asyncore.dispatcher):
    def __init__(self, addr, user_validator=None, ip_validator=None, domain_validator=None,
                 collect_domain_validator=None, mailbox_validator=None, timeout=600):
    ...
    def handle_accept(self):
        ...
        if pair is not None:
            ...
            channel = SMTPChannel(self, conn, addr, user_validator=self.user_validator, ip_validator=self.ip_validator,
                                  ...
                                  collect_domain_validator=self.collect_domain_validator,
                                  mailbox_validator=self.mailbox_validator)
    ...
    def process_message(self, username, uid, mailfrom, rcpttos, collect_rcpttos, data, client_ip, helo, senderip):
        try:
            ...
            if format_ok_addresses:
                self.save_mail(uid, mailfrom, format_ok_addresses, data, senderip)
            if collect_rcpttos:
                for k, v in collect_rcpttos.iteritems():
                    self.save_mail(k, mailfrom, set(v), data, senderip, type='collect')
    ...
    def save_mail(self, uid, mailfrom, rcpttos, data, client_ip, type='relay'):
        ...
        filename = '{},{},{}'.format(uid, mail_id, client_ip)
        ...
        froms_path = settings.DATA_FROMS_PATH if type == 'relay' else settings.DATA_COLLECT_FROMS_PATH
...
if __name__ == '__main__':
    ...
    listen_ports = map(lambda x: int(x), Common.cfgDefault.get('postfix_esmtpd', 'listen_port').split(','))
    ...
    for listen_port in listen_ports:
        SMTPServer((listen_ip, listen_port),
                   ...
                   domain_validator=auth.auth_by_domain, collect_domain_validator=auth.auth_by_collect_domain,
                   mailbox_validator=auth.auth_by_mailbox, timeout=timeout)
