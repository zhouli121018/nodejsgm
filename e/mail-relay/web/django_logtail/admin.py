...
class LogAdmin(admin.ModelAdmin):
    ...
    def update_logtail_files(self):
        self.logtail_files = app_settings.get_logtail_files()
    ...
    def log_context(self, logfile, seek_to):
        context = {}
        seek_to = int(seek_to)
        self.update_logtail_files()
        #logtail_files = app_settings.get_logtail_files()
        try:
            #log_file = app_settings.LOGTAIL_FILES[logfile]
            log_file = self.logtail_files[logfile][1]
        except KeyError:
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        context = dict(
            ...
            logfiles=((l, f[0], f[1]) for l, f
                in self.logtail_files.iteritems()
                if isfile(f[1])
            ),
        )
