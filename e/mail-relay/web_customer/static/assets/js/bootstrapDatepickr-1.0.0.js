;(function ( $, window, document, undefined ) {
	...
	$.extend(Plugin.prototype, {
		...
		init_cal: function(plugin, element, cal_month, cal_year) {
			...
			t += '<th colspan="5" class="bootstrapDatepickr-month month_id-' + $(element).attr('id') + '">' + cal_year + ' - ' + cal_month + '</th>';
		...
		wrapper_clicked: function(e)
		{
			...
			$('body').unbind().on('click', '.day_id-' + $(this).attr('id'), function () {
				if($(this).html() != "&nbsp;"){
					...
					var currentTimestamp = new Date(plugin.settings.current_year, plugin.settings.current_month - 1, plugin.settings.selected_day).getTime();
					...
					$("#" + $(plugin.element).attr('id')).attr('data-time',currentTimestamp);

					//$("#" + $(plugin.element).attr('id')).parents('ul').prev().find('span').html(formatted_date);


					if($(plugin.element).attr('id') == "calendar1"){

					}else if($(plugin.element).attr('id') == "calendar2"){
					...
					setTimeout(function(){
						$("#" + $(plugin.element).attr('id')).parents('ul').parents('.dropdown-hover').addClass('hover');
						$("#" + $(plugin.element).attr('id')).parents('ul').find('input').show();
						$("#" + $(plugin.element).attr('id')).parents('ul').parents('.dropdown-hover').mouseleave(function(){
							$("#" + $(plugin.element).attr('id')).parents('ul').parents('.dropdown-hover').removeClass('hover');
						})
					},40);
				}
			});
			...
			$('body').on('click', '.next_id-' + $(this).attr('id'), function () {
				...
				$(".month_id-" + $(plugin.element).attr('id')).html(settings.current_year + " - " + settings.current_month);
			});
			...
			if(".cal_id-" + $(this).attr('id')=='.cal_id-calendar1'&&!($(".cal_id-" + $(this).attr('id')).hasClass('bootstrapDatepickr-hidden'))){
				$('.cal_id-calendar3').addClass('bootstrapDatepickr-hidden');
			}else if(".cal_id-" + $(this).attr('id')=='.cal_id-calendar3'&&!($(".cal_id-" + $(this).attr('id')).hasClass('bootstrapDatepickr-hidden'))){
				$('.cal_id-calendar1').addClass('bootstrapDatepickr-hidden');
			}else if(".cal_id-" + $(this).attr('id')=='.cal_id-calendar2'&&!($(".cal_id-" + $(this).attr('id')).hasClass('bootstrapDatepickr-hidden'))){
				$('.cal_id-calendar4').addClass('bootstrapDatepickr-hidden');
			}else if(".cal_id-" + $(this).attr('id')=='.cal_id-calendar4'&&!($(".cal_id-" + $(this).attr('id')).hasClass('bootstrapDatepickr-hidden'))){
				$('.cal_id-calendar2').addClass('bootstrapDatepickr-hidden');
			}
