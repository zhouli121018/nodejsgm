...
class TagsForObjectNode(Node):
    ...
    def render(self, context):
        ...
        lists = Tag.objects.get_for_object(self.obj.resolve(context))
        ...
        context[self.context_var] = lists
        return ''
...
def if_exsits_tagitem(obj, tag_type, obj_id):
    if tag_type == 'customer':
        tag_obj = Customer
    elif tag_type == 'template':
        tag_obj = SendTemplate
    elif tag_type == 'address':
        tag_obj = MailList
    ctype = ContentType.objects.get_for_model(tag_obj)
    obj = TaggedItem.objects.filter(tag=obj, content_type=ctype, object_id=obj_id).first()
    return obj
    # return TaggedItem.objects.filter(tag=obj, content_type=ctype, object_id=obj_id).exists()

@register.assignment_tag
def get_children_list(obj, tag_type, obj_id):
    if tag_type == 'customer':
        tag_obj = Customer
    elif tag_type == 'template':
        tag_obj = SendTemplate
        ...
        tag_obj = MailList
    ctype = ContentType.objects.get_for_model(tag_obj)
    objs = Tag.objects.filter(parent=obj)
    return TaggedItem.objects.filter(tag__in=objs, content_type=ctype, object_id=obj_id)
