...
resolv_file = os.path.join(ROOT, '..', 'conf', 'resolv2.conf')
...
def RDnsQuery(ip, domain=None, answers=None, rdtype="a"):
    ...
    try:
        ...
        log.error('RDnsQuery2 error: ip={}, domain: {}, error: {}'.format(ip, domain, e))
        # log.error(traceback.format_exc())
        return False
    for i in r:
        i = str(i)
        if i in answers:
            ...
            log.warning('RDnsQuery2 error: ip={} results={}'.format(ip, list(r)))
    return False
