...
class TagManager(models.Manager):
    ...
    def delete_tag(self, obj, tag_name):
        """
        Associates the given object with a tag.
        """
        tag_names = parse_tag_input(tag_name)
        ...
        try:
            ...
            ctype = ContentType.objects.get_for_model(obj)
            TaggedItem._default_manager.get(
                tag=tag, content_type=ctype, object_id=obj.pk).delete()
        except:
            pass
    ...
    def get_for_object(self, obj):
        ...
        """
        ctype = ContentType.objects.get_for_model(obj)
        lists = self.filter(items__content_type__pk=ctype.pk, items__object_id=obj.pk)
        for l in lists:
            if l.isremark:
                item = TaggedItem.objects.filter(tag=l, content_type=ctype, object_id=obj.pk).first()
                if item.remark:
                    l.name = u'{}(备注：{})'.format(l.name, item.remark)
        return lists
...
class TagCategory(models.Model):
    '''
    name: tag 分类一级菜单，比如：客户来源、客户类型
    category： 分类： 客户、模板、地址池
    Tag：外键 + name 唯一
    '''
    category = models.CharField('category', max_length=10, null=False, blank=False, choices=TAG_CATEGORY)
    name = models.CharField('name', max_length=50, null=False, blank=False)
    order_id = models.IntegerField(u'排序', default=1, null=False, blank=False,help_text=u'界面展现顺序，按照升序排序，比如1,2,3,4,...')
    is_show = models.BooleanField(u'展现到用户界面', default=False, help_text=u'用户界面展现该标签系列，用于精准营销。')

    class Meta:
        managed = False
        unique_together = (('category', 'name'),)
        db_table = 'tagging_category'
...
class Tag(models.Model):
    ...
    A tag.
    """
    parent = models.ForeignKey('self', null=True, blank=True, related_name='children', on_delete=models.SET_NULL, db_index=True)
    category = models.ForeignKey(TagCategory, verbose_name='category', related_name='categories', on_delete=models.CASCADE)
    name = models.CharField(_('name'), max_length=settings.MAX_TAG_LENGTH)
    isremark = models.BooleanField(u'是否进行备注', default=False)
    order_id = models.IntegerField(u'排序', default=99, null=False, blank=False,help_text=u'界面展现顺序，按照升序排序，比如1,2,3,4,...')

    objects = TagManager()

    def get_exists(self):
        return TaggedItem.objects.filter(tag=self).exists()
    ...
    def get_name(self):
        if self.parent:
            return u'{} > {}'.format(self.parent.name, self.name)
        return self.name
