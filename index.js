...
var pool=mysql.createPool({
    host:'120.77.43.40',//qingyuankx 183.131.200.109 // 朝阳 47.95.239.253 //juyou 116.62.56.47 //ningdu 120.77.43.40//qingyuan120.76.100.224 //suzhou 121.196.221.247// songyuan 39.106.132.18 // qingyuan1213 103.73.206.31 //测试 120.79.23.45
    //user:'root',//mahjong
    ...
    if(req.session.user){
        ...
        pool.getConnection((err,conn)=>{
            if(err){
                ...
                conn.query('SELECT m.*,a.uuid,a.nickName,a.roomCard FROM manager m,account a WHERE m.id=? and m.id = a.managerId',[managerId],(err,result)=>{
                    //console.log(result);
                ...
                if(powerId==1){
                    if(inputManagerId){
                        ...
                        if(uuid){
                            ...
                            conn.query('SELECT count(a.uuid) as totalNum FROM (select n.*,IFNULL(sum(p.money),0) as totalMoney from account n LEFT JOIN paylog p on p.uuid=n.uuid where p.payType=0 and p.payTime>? and p.payTime<? group by n.id)a ',[starttime,endtime],(err,result)=>{
                                progress++;
                ...
                if(!uname&&!inviteCode){
                    pool.getConnection((err, conn)=> {
                        if (err) {
                            ...
                            var progress=0;
                            if(inputPowerId){
                                    conn.query('select s.*,a.nickName,a.roomCard,a.Uuid from(SELECT r.*,COUNT(g.id) as agentNum from(SELECT q.*,IFNULL(sum(p.money),0) as totalMoney from(select m.*,count(a.id) as userCounts from (select * from manager where id>3 and power_id=?) m left JOIN account a ON  a.manager_up_id=m.id GROUP BY m.id )q LEFT JOIN paylog p on p.payType=0  and p.payTime>? and p.payTime< ? and p.managerId=q.id GROUP BY q.id)r LEFT JOIN manager g on g.manager_up_id=r.id GROUP BY r.id)s LEFT JOIN account a ON s.id=a.managerId and a.status!=2  ORDER BY totalMoney desc,userCounts desc limit ?,10', [inputPowerId,starttime,endtime,limitstart], (err, result)=> {
                                        //console.log(result);
                                        resultJson.managers=result;
                                        progress++;
                                        if(progress==3){
                                ...
                                conn.query('select count(m.id) as totalNum from manager m where id>3 and power_id=?',[inputPowerId],(err,result)=>{
                                    ...
                                    conn.query('select s.*,a.nickName,a.roomCard,a.Uuid from(SELECT r.*,COUNT(g.id) as agentNum from(SELECT q.*,IFNULL(sum(p.money),0) as totalMoney from(select m.*,count(a.id) as userCounts from (select * from manager where manager_up_id<4 and id>3) m left JOIN account a ON  a.manager_up_id=m.id GROUP BY m.id )q LEFT JOIN paylog p on p.payType=0 and p.payTime>? and p.payTime< ? and p.managerId=q.id GROUP BY q.id)r LEFT JOIN manager g on g.manager_up_id=r.id GROUP BY r.id)s LEFT JOIN account a ON s.id=a.managerId and a.status!=2  ORDER BY totalMoney desc,userCounts desc limit ?,10', [starttime,endtime,limitstart], (err, result)=> {
                                        //console.log(result);
                                        resultJson.managers=result;
                                        progress++;
                                        if(progress==3){
                                ...
                                conn.query('select count(m.id) as totalNum from manager m where m.manager_up_id<4 and id>3',(err,result)=>{
                                    resultJson.totalNum=result[0].totalNum;
                                    progress++;
                                    if(progress==3){
                                        res.json(resultJson);
                    ...
                    pool.getConnection((err, conn)=> {
                        if (err) {
                            ...
                            var progress=0;
                            if(inputPowerId){
                                    conn.query('select s.*,a.nickName,a.roomCard,a.Uuid from(SELECT r.*,COUNT(g.id) as agentNum from(SELECT q.*,IFNULL(sum(p.money),0) as totalMoney from(select m.*,count(a.id) as userCounts from (select * from manager where name=? and inviteCode=? and power_id=?) m left JOIN account a ON  a.manager_up_id=m.id GROUP BY m.id )q LEFT JOIN paylog p on p.payType=0  and p.payTime>? and p.payTime< ? and p.managerId=q.id GROUP BY q.id)r LEFT JOIN manager g on g.manager_up_id=r.id)s LEFT JOIN account a ON s.id=a.managerId and a.status!=2  ORDER BY totalMoney desc,userCounts desc limit ?,10', [uname,inviteCode,inputPowerId,starttime,endtime,limitstart], (err, result)=> {
                                        //console.log(result);
                                        resultJson.managers=result;
                                        progress++;
                                        if(progress==3){
                    ...
                    pool.getConnection((err, conn)=> {
                        if (err) {
                            ...
                            var progress=0;
                            if(inputPowerId){
                                    conn.query('select s.*,a.nickName,a.roomCard,a.Uuid from(SELECT r.*,COUNT(g.id) as agentNum from(SELECT q.*,IFNULL(sum(p.money),0) as totalMoney from(select m.*,count(a.id) as userCounts from (select * from manager where name=? and power_id=?) m left JOIN account a ON  a.manager_up_id=m.id GROUP BY m.id )q LEFT JOIN paylog p on p.payType=0  and p.payTime>? and p.payTime< ? and p.managerId=q.id GROUP BY q.id)r LEFT JOIN manager g on g.manager_up_id=r.id)s LEFT JOIN account a ON s.id=a.managerId and a.status!=2 ORDER BY totalMoney desc,userCounts desc limit ?,10', [uname,inputPowerId,starttime,endtime,limitstart], (err, result)=> {
                                        //console.log(result);
                                        resultJson.managers=result;
                                        progress++;
                                        if(progress==3){
                                ...
                                conn.query('select count(m.id) as totalNum from manager m where name=? and power_id=?',[uname,inputPowerId],(err,result)=>{
                                    ...
                                    conn.query('select s.*,a.nickName,a.roomCard,a.Uuid from(SELECT r.*,COUNT(g.id) as agentNum from(SELECT q.*,IFNULL(sum(p.money),0) as totalMoney from(select m.*,count(a.id) as userCounts from (select * from manager where name=?) m left JOIN account a ON  a.manager_up_id=m.id GROUP BY m.id )q LEFT JOIN paylog p on p.payType=0  and p.payTime>? and p.payTime< ? and p.managerId=q.id GROUP BY q.id)r LEFT JOIN manager g on g.manager_up_id=r.id)s LEFT JOIN account a ON s.id=a.managerId and a.status!=2 ORDER BY totalMoney desc,userCounts desc limit ?,10', [uname,starttime,endtime,limitstart], (err, result)=> {
                                        //console.log(result);
                                        resultJson.managers=result;
                                        progress++;
                                        if(progress==3){
                    ...
                    pool.getConnection((err, conn)=> {
                        if (err) {
                            ...
                            var progress=0;
                            if(inputPowerId){
                                    conn.query('select s.*,a.nickName,a.roomCard,a.Uuid from(SELECT r.*,COUNT(g.id) as agentNum from(SELECT q.*,IFNULL(sum(p.money),0) as totalMoney from(select m.*,count(a.id) as userCounts from (select * from manager where inviteCode=? and power_id=?) m left JOIN account a ON  a.manager_up_id=m.id GROUP BY m.id )q LEFT JOIN paylog p on p.payType=0  and p.payTime>? and p.payTime< ? and p.managerId=q.id GROUP BY q.id)r LEFT JOIN manager g on g.manager_up_id=r.id GROUP BY r.id)s LEFT JOIN account a ON s.id=a.managerId and a.status!=2  ORDER BY totalMoney desc,userCounts desc limit ?,10',[inviteCode,inputPowerId,starttime,endtime,limitstart], (err, result)=> {
                                        //console.log(result);
                                        resultJson.managers=result;
                                        progress++;
                                        if(progress==3){
                                ...
                                conn.query('select count(m.id) as totalNum from manager m where inviteCode=? and power_id=?',[inviteCode,inputPowerId],(err,result)=>{
                                    ...
                                    conn.query('select s.*,a.nickName,a.roomCard,a.Uuid from(SELECT r.*,COUNT(g.id) as agentNum from(SELECT q.*,IFNULL(sum(p.money),0) as totalMoney from(select m.*,count(a.id) as userCounts from (select * from manager where inviteCode=?) m left JOIN account a ON  a.manager_up_id=m.id GROUP BY m.id )q LEFT JOIN paylog p on p.payType=0  and p.payTime>? and p.payTime< ? and p.managerId=q.id GROUP BY q.id)r LEFT JOIN manager g on g.manager_up_id=r.id GROUP BY r.id)s LEFT JOIN account a ON s.id=a.managerId and a.status!=2  ORDER BY totalMoney desc,userCounts desc limit ?,10',[inviteCode,starttime,endtime,limitstart], (err, result)=> {
                                        //console.log(result);
                                        resultJson.managers=result;
                                        progress++;
                                        if(progress==3){
            ...
            if(!uname&&!inviteCode){
                ...
                pool.getConnection((err, conn)=> {
                    if (err) {
                        ...
                        var progress=0;
                            conn.query('select s.*,a.nickName,a.roomCard,a.Uuid from(SELECT r.*,COUNT(g.id) as agentNum from(SELECT q.*,IFNULL(sum(p.money),0) as totalMoney from(select m.*,count(a.id) as userCounts from (select * from manager where name=? and manager_up_id=?) m left JOIN account a ON  a.manager_up_id=m.id GROUP BY m.id )q LEFT JOIN paylog p on p.payType=0  and p.payTime>? and p.payTime< ? and p.managerId=q.id GROUP BY q.id)r LEFT JOIN manager g on g.manager_up_id=r.id GROUP BY r.id)s LEFT JOIN account a ON s.id=a.managerId and a.status!=2  ORDER BY totalMoney desc,userCounts desc limit ?,10', [uname,managerId,starttime,endtime,limitstart], (err, result)=> {
                                //console.log(result);
                                resultJson.managers=result;
                                progress++;
                                if(progress==3){
